...
package projet_bi_4.verification_annee_fichier_0_1;
...
/**
 * Job: verification_annee_fichier Purpose: verifie l'année dans le nom du fichier<br>
 * Description: Vérifie que l'année est bien présente en fin de nom de fichier et qu'elle est valide (=année courante sur 4 chiffres) <br>
 * @author user@talend.com
 * @version 7.2.1.20190325_1906-M4
 * @status 
 */
public class verification_annee_fichier implements TalendJob {
...
protected static void logIgnoredError(String message, Throwable cause) {
	...
	public class ContextProperties extends PropertiesWithType {
		...
		public void synchronizeContext(){
			...
			if(verifier_mois != null){
				
					this.setProperty("verifier_mois", verifier_mois.toString());
...
public Boolean getVerifier_mois(){
	return this.verifier_mois;
}
	...
	private final String jobName = "verification_annee_fichier";
...
private RunStat runStat = new RunStat();
	...
	StatCatcherUtils talendStats_STATS = new StatCatcherUtils("_29_MUGOIEem3g57GDjk1DQ", "0.1");
	MetterCatcherUtils talendMeter_METTER = new MetterCatcherUtils("_29_MUGOIEem3g57GDjk1DQ", "0.1");
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				verification_annee_fichier.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(verification_annee_fichier.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
try {
	annee =  projetBi.getYearFromFile(context.nom_fichier);
}
catch( Exception ex) {
//le dernier paramètre n'est pas un entier
throw new Exception("L'année dans le nom de fichier '"+  context.nom_fichier +"' n'est pas valide. Format attendu : *_yyyy.xls");
}

//l'annee n'est pas sur 4 chiffres
if (annee < 1000) {
throw new Exception("L'année dans le nom de fichier '"+  context.nom_fichier +"' n'est pas valide. Format attendu : *_yyyy.xls.");
}

System.out.println(annee);
System.out.println(projetBi.getCurrentYear());
//ce n'est pas l'année courante
if ( annee != projetBi.getCurrentYear()) throw new Exception("L'année dans le nom de fichier '"+  context.nom_fichier +"' n'est pas l'année courante.");



if (context.verifier_mois) {
	
	try {
		mois = projetBi.getMonthFromFile(context.nom_fichier);
	}
	catch(Exception ex) {
		throw new Exception("Impossible d'extraire le mois dans le nom de fichier '"+  context.nom_fichier +"'. Format attendu : *_Mois_yyyy.xls");	
	}

	if (!projetBi.getNormalizedMonthList(true).contains(mois)) {
		throw new Exception("Le mois dans le nom de fichier '"+  context.nom_fichier +"' est invalide. Format attendu : *_Mois_yyyy.xls, avec Mois sans accent et première lettre en majuscule.");	
	}
}
...
public static class row_talendStats_STATSStruct implements routines.system.IPersistableRow<row_talendStats_STATSStruct> {
    final static byte[] commonByteArrayLock_PROJET_BI_4_verification_annee_fichier = new byte[0];
    static byte[] commonByteArray_PROJET_BI_4_verification_annee_fichier = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_PROJET_BI_4_verification_annee_fichier.length) {
				if(length < 1024 && commonByteArray_PROJET_BI_4_verification_annee_fichier.length == 0) {
   					commonByteArray_PROJET_BI_4_verification_annee_fichier = new byte[1024];
				} else {
   					commonByteArray_PROJET_BI_4_verification_annee_fichier = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_PROJET_BI_4_verification_annee_fichier, 0, length);
			strReturn = new String(commonByteArray_PROJET_BI_4_verification_annee_fichier, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_PROJET_BI_4_verification_annee_fichier) {
 ...
 */
    ...
    public static void main(String[] args){
        final verification_annee_fichier verification_annee_fichierClass = new verification_annee_fichier();

        int exitCode = verification_annee_fichierClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = verification_annee_fichier.class.getClassLoader().getResourceAsStream("projet_bi_4/verification_annee_fichier_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = verification_annee_fichier.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            ...
            class ContextProcessing {
                private void processContext_0() {
                        ...
                        context.setContextType("verifier_mois", "id_Boolean");
                            try{
                                context.verifier_mois=routines.system.ParserUtils.parseTo_Boolean (context.getProperty("verifier_mois"));
                            } catch(NumberFormatException e){
                                System.err.println(String.format("Null value will be used for context parameter %s: %s", "verifier_mois", e.getMessage()));
                                context.verifier_mois=null;
                            }
        ...
        if (parentContextMap != null && !parentContextMap.isEmpty()) {if (parentContextMap.containsKey("nom_fichier")) {
            ...
            }if (parentContextMap.containsKey("verifier_mois")) {
                context.verifier_mois = (Boolean) parentContextMap.get("verifier_mois");
...
this.globalResumeTicket = true;//to run tPostJob
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : verification_annee_fichier");
        }
...
/************************************************************************************************
 *     92434 characters generated by Talend Open Studio for Data Integration 
